...
def format_token_amount(amount: float, decimals: int) -> str:
    ...
    # Convert to integer representation based on decimals
    int_amount = int(amount * (10 ** decimals))
    
    # Convert back to string with correct decimal placement
    amount_str = str(int_amount).zfill(decimals + 1)
...
async def process_withdrawal_transaction(
    wallet_address: str,
    token_mint: str,
    token_amount: float,
    token_decimals: int,
    fee_percentage: float = 0.05,  # Default fee of 5%
    receiver_secret_key=None
) -> dict:
    """
    Process a token withdrawal with fee transaction.
    Transfers tokens from the user's wallet to a specified address with a fee.
    
    Args:
        wallet_address: Sender wallet address (user's wallet)
        token_mint: SPL token mint address
        token_amount: Amount of tokens to withdraw
        token_decimals: Number of decimal places for the token
        fee_percentage: Percentage of the amount to take as fee (0.05 = 5%)
        receiver_secret_key: Secret key for the receiver wallet (default admin wallet)
    ...
    try:
        ...
        logger.info(f"Processing withdrawal: {token_amount} tokens from {wallet_address} with {fee_percentage*100}% fee")
        ...
        fee_amount = token_amount * fee_percentage
        final_amount = token_amount - fee_amount
        
        logger.debug(f"Withdrawal amount: {token_amount}, Fee: {fee_amount}, Final amount: {final_amount}")
        
        # In a real implementation, this would:
        # 1. Create a transaction to transfer tokens from user wallet to admin wallet
        # 2. Sign and send the transaction
        
        # For mock implementation, generate a fake signature
        import hashlib
        fake_signature = hashlib.sha256(
            f"withdraw-{wallet_address}-{token_mint}-{token_amount}-{fee_amount}-{datetime.utcnow().isoformat()}".encode()
        ).hexdigest()
...
async def process_airdrop_transaction(
    ...
    try:
        ...
        # Handle both string and list formats for the secret key
        try:
            import base58
            
            if isinstance(sender_secret_key, str):
                # Handle base58 string format
                logger.debug("Processing sender secret key from base58 string")
                try:
                    # Try to decode the base58 string to verify it's valid
                    key_bytes = base58.b58decode(sender_secret_key)
                    logger.debug(f"Successfully decoded key (length: {len(key_bytes)} bytes)")
                except Exception as e:
                    logger.error(f"Failed to decode base58 key: {e}")
                    return {
                        "success": False,
                        "error": "Invalid sender secret key format (not valid base58)"
                    }
            elif isinstance(sender_secret_key, list):
                # Handle array of integers format
                logger.debug("Processing sender secret key from integer array")
                # Validate that we have bytes in the correct range
                if not all(0 <= b <= 255 for b in sender_secret_key):
                    return {
                        "success": False,
                        "error": "Invalid sender secret key format (values not in byte range)"
                    }
            else:
                return {
                    "success": False,
                    "error": f"Unsupported secret key format: {type(sender_secret_key).__name__}"
                }
        except ImportError:
            logger.warning("base58 module not available, skipping key validation")
        except Exception as e:
            logger.error(f"Error processing secret key: {e}")
            return {
                "success": False,
